...
export const getshopRents = (result) => {
    db.query("SELECT * FROM  rent_for_shop", (err,results)=> {
        if (err){
            console.log(err);
            result(err,null);
        }else{
...
export const getshopRentsById = (id,result) => {
    db.query("SELECT * FROM rent_for_shop AS r,market_or_streets AS ms WHERE r.rent_cus_nic = ? AND r.rent_shop_place=ms.ms_id",[id], (err,results)=> {
        if (err){
            console.log(err);
            result(err,null);
        }else{
